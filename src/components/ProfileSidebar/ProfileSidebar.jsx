...
export default function ProfileSidebar() {
  ...
  // Close sidebar drawer on page routing
  useEffect(() => {
    setIsOpen(false)
  }, [location.pathname])
  ...
  return (
    <>
      ...
      <button 
        className={`ps-toggle-btn${isOpen ? ' active' : ''}`} 
      ...
      {isOpen && (
        <div className="ps-overlay" onClick={() => setIsOpen(false)} />
      )}

      <aside className={`ps-sidebar${isOpen ? ' ps-open' : ''}`} role="navigation" aria-label="Profile navigation">
        {/* User Info */}
